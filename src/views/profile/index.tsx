...
function Profile() {
  ...
  const getProfile = () => {
    ...
    let historyTransaction = JSON.parse(
      sessionStorage.getItem(`order-${userName}`) || "[]"
    );
  ...
  return (
    <div className="md:grid md:grid-cols-2 md:gap-10 md:p-8 p-5 text-left">
      <div className="mb-10">
        ...
        <img
          src={require("../../assets/img/company-logo.jpeg")}
          alt="company-logo"
          className="w-32 h-32 mx-auto my-12"
        />
        <p className="mb-5">
          Jl. Kesehatan Raya No.12RT.1/RW.11, Bintaro, Kec. Pesanggrahan, Kota
          Jakarta Selatan, Daerah Khusus Ibukota Jakarta 12330
        </p>
        <div className="mb-5">
          <p className="font-bold">About {username}</p>
          <p>{username} is a dummy profile for passed the test bababos</p>
        </div>
        <div className="flex">
        ...
        <div className="flex">
          ...
          <span>https://www.dummycompany.com</span>
        </div>
        <div className="flex">
      ...
      <div>
        ...
        {transaction &&
          transaction.map((e: TransactionHistoryProps, index: number) => (
            <div className="md:p-3 shadow-md" key={index}>
              <div
                className={`flex ${
                  e.isOpenDetail ? "border-b border-current-500 pb-3" : ""
                }`}
                onClick={() => openDetail(index)}>
                <img
                  src={
                    (Object.entries(e.orders)[0][1] as ProductProps[])[0].image
                  }
                  className="w-12 h-12 object-fit self-center mr-5 "
                  alt=""
                />
                <div className="self-center">
                  <p className="text-sm font-bold">{e.orderName}</p>
                  <p className="text-xs">
                    {moment(e.createDate).format("DD MMM yyyy")}
                  </p>
                </div>
                <div className="ml-auto self-center">
                  {e.isOpenDetail ? <IoIosArrowUp /> : <IoIosArrowDown />}
                </div>
              </div>
              {e.isOpenDetail &&
                Object.entries(e.orders).map((o: any, idx: number) => (
                  <div className="flex mt-5 px-3" key={idx}>
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold truncate w-[200px]">
                        {o[1][0].title}
                      </p>
